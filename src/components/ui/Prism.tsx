...
export default function Prism({
  ...
  const iterations = quality === "low" ? 48 : quality === "high" ? 100 : 70;
  const maxDpr = quality === "low" ? 1 : quality === "high" ? 2 : 1.5;
  ...
  useEffect(() => {
    ...
    const fragment = `
      ...
      float sdPyramidUpInv(vec3 p){ float oct=sdOctaAnisoInv(p); return max(oct,-p.y); }
    ...
    const wX = (0.3 + rnd() * 0.6); const wY = (0.2 + rnd() * 0.7); const wZ = (0.1 + rnd() * 0.5);
    ...
    function render(t: number) {
      ...
      if (animationType === "hover") {
        ...
        if (NOISE < 1e-6 && Math.abs(yaw-targetYaw)<1e-4 && Math.abs(pitch-targetPitch)<1e-4 && Math.abs(roll)<1e-4) cont = false;
      } else if (animationType === "3drotate") {
        const ts = time * TS;
      ...
      if (cont) raf = requestAnimationFrame(render); else raf = 0;
    }
    ...
    if (suspendWhenOffscreen) {
      ...
      io.observe(container); startRAF(); c.__prismIO = io;
    } else {
      startRAF();
    }
    ...
    return () => {
      ...
      if (suspendWhenOffscreen && c.__prismIO) { c.__prismIO.disconnect(); delete c.__prismIO; }
      if (gl.canvas.parentElement === container) container.removeChild(gl.canvas);
    };
  }, [height, baseWidth, animationType, glow, noise, offset?.x, offset?.y, scale, transparent, hueShift, colorFrequency, timeScale, hoverStrength, inertia, bloom, suspendWhenOffscreen, quality]);
